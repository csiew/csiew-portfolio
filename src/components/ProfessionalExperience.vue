      ...
      <transition name="fade">
        <div v-show="isVisible.timelineSection === true" class="grid grid-col-1 grid-gap-xl align-center justify-center width-full margin-auto-horizontal padding-m">
          <div class="grid-item flex-inline flex-flow-column aligh-center justify-center">
            ...
            <div class="grid grid-col-1 grid-gap-s">
              <div v-for="contentParagraph in this.sections[state.currentSectionId].content" v-bind:key="contentParagraph[0] + contentParagraph.length" class="grid-item card card-marked padding-s padding-none-top padding-none-bottom">
                <span v-html="this.formatMarkdown(contentParagraph)"></span>
              </div>
            </div>
          </div>
        </div>
