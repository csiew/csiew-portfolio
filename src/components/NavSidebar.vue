<template>
  <div id="navSidebarMenu" class="vstack z-index-50 position-fixed anchor-left width-auto align-start noselect" v-bind:class="isVisible.sidebar === true ? 'height-full ' + (isMobile === true ? 'nav vstack-reverse anchor-bottom width-full' : 'nav-pill nav-pill-open nav-shadow nav-border-right anchor-top justify-space-between') : 'height-auto ' + (isMobile === true ? 'vstack-reverse anchor-bottom width-full' : 'nav-pill nav-shadow anchor-top')">
    <div class="z-index-200 hstack align-center" v-bind:class="isMobile === true ? 'nav nav-border-top justify-space-between width-full font-scale-xl' : 'nav-pill-header justify-start width-auto padding-xxs'">
      ...
      <transition name="fade">
        <button v-show="helloIsVisible === false" class="grid-item button-hv-reveal cursor-pointer padding-xs line-height-1" v-bind:class="isMobile === true ? 'margin-s-right margin-auto-left font-scale-xxs' : 'margin-s-right margin-xs-left font-scale-s'" v-on:click="goToSection('top'); isVisible.sidebar = false">Back to top</button>
      </transition>
    </div>
    <div v-if="isVisible.sidebar === true" class="vstack height-full" v-bind:class="isMobile === true ? 'align-center justify-center width-full padding-none font-scale-l' : 'width-auto justify-space-between'">
      <div class="card-list overflow-y-auto" v-bind:class="isMobile === true ? 'width-full height-auto margin-auto padding-m-top padding-m-bottom' : ''">
        <ul v-bind:class="isMobile === true ? 'margin-auto-top margin-auto-bottom' : ''">
          <li v-for="item in sections" v-bind:key="item.id" v-show="!item.hide" class="cursor-pointer" v-bind:class="isMobile === true ? 'width-full padding-xs hstack align-center justify-center font-scale-l' : 'padding-m-left padding-s-right padding-xs-top padding-xs-bottom hstack align-start justify-center'" v-on:click="goToSection(item.id); isVisible.sidebar = false">
            {{ item.label }}
          </li>
        </ul>
      </div>
